...
class BayesianNetwork:
    ...
    def sorted_nodes(self):
        ...
        for var in self.variables.values():
            in_degree.setdefault(var, 0)
            for children in self.edges.values():
                if var in children:
                    in_degree[var] = in_degree.get(var) + 1

        no_indegree = []
        for var in in_degree:
            if in_degree[var] == 0:
                no_indegree.append(var)

        sorted_list = []

        while no_indegree:
            var = no_indegree.pop(0)
            sorted_list.append(var)
            for child in self.edges[var]:
                in_degree[child] = in_degree.get(child) - 1
                if in_degree[child] == 0:
                    no_indegree.append(child)
...
class InferenceByEnumeration:
    ...
    def _enumeration_ask(self, X, evidence):
        ...
        q = []
        for state in range(self.bayesian_network.variables[X].no_states):
            new_evidence = evidence.copy()
            new_evidence[X] = state
            q.append(self._enumerate_all(self.bayesian_network.sorted_nodes(), new_evidence))
        tot = 0
        for i in q:
            tot += i
        q = np.array(q)/tot
    ...
    def _enumerate_all(self, vars, evidence):
        ...
        if y.name in evidence.keys():
            return y.probability(evidence[y.name], evidence) * self._enumerate_all(vars.copy(), evidence)
        else:
            tot = 0
            for i in range(y.no_states):
                evidence[y.name] = i
                tot += y.probability(i, evidence) * self._enumerate_all(vars.copy(), evidence.copy())
            return tot
...
def monty_hall():
    ...
    alt1 = Variable("Prize", 3, [[1/3], [1/3], [1/3]])
    alt2 = Variable("ChosenByGuest", 3, [[1/3], [1/3], [1/3]])
    alt3 = Variable("OpenedByHost", 3, [[0, 0, 0, 0, 0.5, 1, 0, 1, 0.5],
    ...
    bn = BayesianNetwork()
    bn.add_variable(alt1)
    bn.add_variable(alt2)
    bn.add_variable(alt3)
    bn.add_edge(alt1, alt3)
    bn.add_edge(alt2, alt3)
    inference = InferenceByEnumeration(bn)
    posterior = inference.query("Prize", {"ChosenByGuest": 0, "OpenedByHost": 2})

    print(f"Probability distribution, P({alt1.name} | {alt2.name} = 1 , {alt3.name} = 3)")
    print(posterior)
    pass
